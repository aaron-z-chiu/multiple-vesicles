...
void source(float ***c_old, float ***o1, float ***o2, float ***o3, float ***src_c, float ***src_mu)
{
    ...
    ijkloop
    {
        ...
        src_c[i][j][k] = (c_old[i][j][k] / dt) + (3.0 / (sqrt(2) * gam)) * oo1[i][j][k] * (C1 * (oo2[i][j][k] * oo2[i][j][k] - 1) + C2 * (oo3[i][j][k] * oo3[i][j][k] - 1)) - sig * (6.0 * (0.5 * (oo1[i][j][k] + 1.0)) - 6 * pow(0.5 * (oo1[i][j][k] + 1.0), 2)) * (V - V0);
